...
// This function safely gets the API key without crashing the app if `process` is undefined.
const getApiKey = () => {
  try {
    ...
    console.warn("Could not access environment variables. API key might be missing.");
    return null;
  }
};
...
const App = () => {
  ...
  // Initialize ai instance; will be null if API key is not found.
  const ai = useMemo(() => {
